...
export class BillDetailPage {
  ...
  ionViewDidLoad() {
    ...
    this.grossamount = this.Bills.totalamount + Number(this.Bills.labourcharge);
  }
        ...
        this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.WRITE_EXTERNAL_STORAGE).then(
          result => {
            ...
            if (result.hasPermission) {
              ...
              var folderpath = (this.file.externalRootDirectory || this.file.dataDirectory) + 'TechGoldApp/';
  ...
  savebase64AsPDF(folderpath, filename, content, contentType) {
    ...
    self.file.writeFile(folderpath, filename, DataBlob, { replace: true }).then((success) => {
      ...
      let toast = this.toastCtrl.create({
        message: 'Bill downloaded successfully, file location is inside filemanager/TechGoldApp',
        position: 'top',
        duration: 3000
      });
      toast.present();
